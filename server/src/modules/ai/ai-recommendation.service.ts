...
export class AIRecommendationService {
    ...
    for (const resource of resources) {
      ...
      if (score > 0.3) { // Only include resources with decent scores
        recommendations.push({
          resourceId: (resource as any)._id.toString(),
          score,
  ...
  private calculatePerformanceScore(requirements: any, resource: Resource): number {
    ...
    if (requirements.gpu && (resource as any).gpu) {
      score += 1;
      factors++;
    }
  ...
  private calculateCostScore(
    ...
    const estimatedCost = ((resource as any).pricePerHour || 0) * (request.duration || 1);
  ...
  private calculateCompatibilityScore(request: RecommendationRequest, resource: Resource): number {
    ...
    if (!request.region || (resource as any).region === request.region) {
      score += 1;
      factors++;
    }
  ...
  private calculateEstimatedCost(resource: Resource, duration: number): number {
    return ((resource as any).pricePerHour || 0) * duration;
  }
  ...
  private generateReasons(
    ...
    if ((resource as any).gpu && request.requirements.gpu) {
      reasons.push('GPU acceleration available');
    }

    if ((resource as any).region === request.region) {
  ...
  private extractUserPreferences(bookingHistory: any[], analytics: any[]) {
    const preferences = {
      ...
      preferredInstanceTypes: [] as string[],
      averageBudget: 0,
      commonWorkloads: [] as string[],
    };
  ...
  private isWorkloadCompatible(workloadType: string, resourceType: string): boolean {
    ...
    return (compatibility as any)[workloadType]?.includes(resourceType) || false;
  }
  ...
  private getSecurityLevelMatch(requiredLevel: string, resource: Resource): number {
    ...
    return (securityLevels as any)[requiredLevel] || 0.5;
  }
}
