...
export class AnalyticsService {
  ...
  async getResourceAnalytics(resourceId: string, timeRange: string = '30d'): Promise<ResourceAnalytics> {
    ...
    try {
      ...
      const totalCost = bookings.reduce((sum: number, booking: any) => sum + (booking.totalCost || 0), 0);
  ...
  async getUserAnalytics(userId: string, timeRange: string = '30d'): Promise<UserAnalytics> {
    ...
    try {
      ...
      const totalSpent = bookings.reduce((sum: number, booking: any) => sum + (booking.totalCost || 0), 0);
      ...
      // Favorite providers
      const providerStats = bookings.reduce((acc: Record<string, any>, booking: any) => {
        const provider = booking.resourceId?.provider || 'unknown';
        if (!acc[provider]) {
          acc[provider] = { count: 0, spent: 0 };
        }
        acc[provider].count++;
        acc[provider].spent += (booking.totalCost || 0);
        return acc;
      ...
      const resourceTypeStats = bookings.reduce((acc: Record<string, any>, booking: any) => {
