...
export class CloudProviderService {
  ...
  async createAWSInstance(specs: any): Promise<string> {
    return this.awsService.provisionResource(specs);
  }

  async createAzureVM(specs: any): Promise<string> {
    return this.azureService.provisionResource(specs);
  }

  async createGCPInstance(specs: any): Promise<string> {
    return this.gcpService.provisionResource(specs);
  }

  async terminateAWSInstance(instanceId: string): Promise<void> {
    return this.awsService.decommissionResource(instanceId);
  }

  async deleteAzureVM(resourceGroup: string, vmName: string): Promise<void> {
    return this.azureService.decommissionResource(vmName, resourceGroup);
  }

  async deleteGCPInstance(projectId: string, zone: string, instanceName: string): Promise<void> {
    return this.gcpService.decommissionResource(instanceName, zone);
  }
}
