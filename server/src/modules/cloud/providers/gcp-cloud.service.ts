...
export class GCPCloudService {
  ...
  private initializeClients() {
    ...
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const GCPMonitoring = require('@google-cloud/monitoring').MetricServiceClient;
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const GCPComputeLib = require('@google-cloud/compute');
    const ComputeClass = GCPComputeLib.default || GCPComputeLib;

    try {
      this.compute = new ComputeClass({
        projectId: this.projectId,
        keyFilename,
      });
    } catch { this.compute = null; }

    try {
      this.monitoring = new GCPMonitoring({
        projectId: this.projectId,
        keyFilename,
      });
    } catch { this.monitoring = null; }
